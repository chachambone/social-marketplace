...
export class ItemsGrid extends LitElement {
  ...
  render() {
    ...
    if (this.error) {
      return html`<div class="error"><p>‚ùå ${this.error}</p><button @click=${this.loadItems}>Try Again</button></div>`;
    }

    if (this.items.length === 0) {
      return html`<div class="empty"><p>Ì≥¶ No items found</p></div>`;
    }

    return html`
      <div class="search-bar">
        <input type="text" class="search-input" placeholder="Ì¥ç Search items..." .value=${this.searchTerm} @input=${this.handleSearch}>
      </div>
      <div class="grid">
